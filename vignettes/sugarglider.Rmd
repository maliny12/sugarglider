...
```{r, eval=FALSE}
# Generate a list of unique train stations that service both metro and vline
both <- train |> filter(mode == "both")

# Generate a list of unique train stations
df_station <- both$station_name |> unique()

# Generate PNG of all the ribbon glyph
purrr::map(1:length(df_station), function(i) {
  dt <- both |> filter(station_name == df_station[i])
  p <- dt |>
...
![](../man/figures/leaflet-screenshot.png)
