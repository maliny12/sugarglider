...
ribbon provides a ggplot2 extension for producing glyph plots with `geom_glyph_ribbon()` and 
`geom_glyph_segment()`.
...
Options allow you to modify the behavior of `ribbon` to fit the specific needs of your figure. These are all global options that affect every glyph.
...
|`width`         | `ggplot2::rel(3)`          |  Width of the glyph.
|`height`     | `ggplot2::rel(2)`     | Height of the glyph.
|`global_rescale`      | `TRUE`        | Determines whether rescaling is applied globally across all glyphs or individually for each glyph

### Aesthetics

`ribbon` provides the same aesthetics for `geom_glyph_ribbon()` and `geom_glyph_segment()` as those available in `geom_ribbon()` or `geom_segment()`, while also introducing additional unique options.
...
The National Oceanica and Atmosphere Administration (NOAA) provides weather data from stations all over Australia. The `aus_temp` dataset contains climate variables such as precipitation, and temperature from 29 different weather stations in the year 2020. For this example, we are interested in finding out the 
